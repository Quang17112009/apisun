...
def create_app():
    ...
    # --- Cấu hình API URL ---
    app.API_URL = os.getenv("API_URL", "https://wanglinapiws.up.railway.app/api/taixiu") # URL API
    ...
    def fetch_data_from_api():
        while True:
            try:
                ...
                # Định dạng dữ liệu từ API: {"Ket_qua":"Xỉu","Phien":2694988,...}
    ...
    def get_taixiu_prediction():
        with app.lock:
            if len(app.history) < 2:
                return jsonify({"error": "Chưa có đủ dữ liệu lịch sử để dự đoán."}), 500
        ...
        # Chỉ học khi có kết quả mới cho phiên đã dự đoán và đó là phiên mới nhất trong lịch sử
        if (last_prediction_copy and 
            history_copy and 
            last_prediction_copy['session'] == history_copy[-1]['phien'] + 1): # Dự đoán cho phiên N, kết quả hiện tại là N-1
            ...
            logging.info(f"Learned from session {history_copy[-1]['phien']}. Prediction was {last_prediction_copy['prediction']}, actual was {actual_result}. Pattern: {last_prediction_copy['pattern']}")
            # Đặt lại last_prediction sau khi học xong để tránh học lại cùng một phiên
            app.last_prediction = None 
        ...
        # Nếu độ tin cậy thấp và không phải là do bẻ cầu, hiển thị "Đang phân tích"
        if confidence < 70.0 and "Bẻ cầu" not in pattern_str: # Ngưỡng tin cậy thấp
             prediction_display = f"Đang phân tích"
             # logging.info(f"Confidence {confidence:.1f}% is low, showing 'Đang phân tích'.")
